...
class QuoteModel {
    ...
    public function getByUser($userId, $filters = []) {
        try {
            ...
            // Mapear estados de BD antigua a nuevos estados
            $reverseStatusMapping = [
                'approved' => 'delivered', // Mapear aprobada a entregada
                'sent' => 'sent',
                'draft' => 'draft'
            ];
            ...
            foreach ($quotes as &$quote) {
                if (isset($quote['status'])) {
                    $quote['status'] = $reverseStatusMapping[$quote['status']] ?? $quote['status'];
                }
            }
    ...
    public function update($id, $data, $userId) {
        try {
            ...
            // Mapear nuevos estados a estados compatibles con BD antigua temporalmente
            $statusMapping = [
                'delivered' => 'approved', // Temporal: mapear entregada a aprobada
                'sent' => 'sent',
                'draft' => 'draft'
            ];
    ...
    public function getStatsByUser($userId) {
        try {
            ...
            return $stmt->fetch(PDO::FETCH_ASSOC) ?: [
                'total' => 0, 'draft' => 0, 'sent' => 0, 'approved' => 0, 'rejected' => 0,
                'approved_amount' => 0, 'total_amount' => 0
            ];
        } catch (Exception $e) {
